...
def format_significant(value, minimum, maximum):
    """Format a number with specified significant digit boundaries"""
    exp = value.adjusted()
    scale = maximum - 1 - exp
    digits = str(value.scaleb(scale).quantize(ONE, ROUND_HALF_EVEN))
    if scale <= 0:
        ...
        result = "{}.{:0<{}}{}{}".format(intpart or '0', '', -min(exp + 1, 0),
                                         fracpart, fracextra).rstrip('.')
    return result
...
class NumberPattern(object):
    ...
    def apply(self, value, locale, currency=None, force_frac=None):
        ...
        if self.exp_prec: # Scientific notation
            ...
            exp = abs(exp)
            number = u'%s%s%s%s' % \
                 (format_significant(value, frac_prec[0], frac_prec[1]),
            ...
            text = format_significant(abs(value),
                                      self.int_prec[0],
                                      self.int_prec[1])
